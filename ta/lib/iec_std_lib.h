...
static inline div_t div(long a, long b){
    div_t rst;
    rst.quot = a / b;
    rst.rem = a % b;
    return rst;
...
static inline double atof(const char* str){
    ...
    if(*str_p == '+'){ // start with plus
        sign = 1;
        str_p++;
        st = INTG;
    } else if (*str_p == '-'){ // start with minus
        sign = -1;
        str_p++;
        st = INTG;
    } else if (isdigit(*str_p)){ // start with digit
        st = INTG;
    } else {
        EMSG("Failed to parse the string!");
        return 0; // return 0 seems to be the most safe option
    }
    // then stop when face a space
    while(!isspace(*str_p)){
        if(isdigit(*str_p)){
            if(st == INTG){
                if(integer_part > (UINT64_MAX - (*str_p - '0'))/10){ // integer part may overflow
                    EMSG("integer part overflow!");
                } else {
                    integer_part = integer_part*10 + (*str_p - '0');
                }
            } else if (st == FRAC){
                if(fractional_part > (UINT64_MAX - (*str_p - '0'))/10){ // frac part may overflow
                    break;
                } else {
                    fractional_part = fractional_part*10 + (*str_p - '0');
                }
            }
        } else {
            if(*str_p == '.'){
                ...
                break; // we regard other characters as termination
            }
        }
        str_p++;
    }
...
static inline void __iec_error(void) {
  ...
  DMSG("IEC 61131-3 runtime error.\n");
  /*exit(1);*/
}
